...
# 예시 MBTI 진로 추천 데이터 (INTJ, INFP만 적용)
mbti_profiles = {
    "INTJ": {
...
st.markdown("단순한 성격 분석을 넘어서, **당신다운 진로**를 함께 상상해봐요!")
    ...
    for field_name, jobs in profile["fields"].items():
        ...
        for job_title, job_reason in jobs:
            st.markdown(
                ...
                f"<div style='font-size:17px; font-weight:600;'>{job_title}</div>"
                f"<div style='font-size:13px; color:gray; padding-left:1em;'>{job_reason}</div>"
                f"</div>",
